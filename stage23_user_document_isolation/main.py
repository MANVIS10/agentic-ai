"""
Stage 23: isolate uploaded documents (and their pgvector chunks) per user,
so User A can never retrieve User B's documents - Stage 20-22 stored and
searched every uploaded document in one shared, unowned pool, which meant
the Knowledge Agent could quote anyone's upload back to anyone who asked a
semantically close-enough question.

New concept vs. Stage 1-22:
- A tool bound to an LLM can read trusted, server-side context that the
  model itself can never see or set. search_uploaded_documents's `query`
  argument is still fully model-controlled (unchanged from Stage 22), but
  its new `user_id` argument is populated by ToolNode FROM THE GRAPH STATE
  via langgraph.prebuilt.InjectedState - bind_tools excludes it from the
  schema the LLM sees entirely. This closes the obvious alternative design
  (a plain `user_id: str` tool argument) that would let a compromised or
  merely confused model call the tool with someone else's user_id.
- user_id is otherwise just one more caller-supplied identifier flowing
  through explicit state, the same way `question`/`thread_id` already do:
  PlannerState -> CriticState -> this stage's new KnowledgeState. No new
  plumbing mechanism, just one more field threaded through graphs that
  already pass state between each other by function call (Stage 17/22).
- This is NOT authentication. user_id is a caller-supplied string trusted
  at face value - nothing here verifies the caller "is" the user_id it
  claims, the same trust model this project has always used for
  thread_id. See .claude/spec/stage23_user_document_isolation_spec.md §3
  for why real auth is explicitly out of scope for this stage.
...
checkpointing, per-thread_id locking, /health, /approve, /reject,
/documents/backfill-embeddings - is copied verbatim from Stage 22. Nothing
about the graph's shape, routing, or those routes' behavior changed.

Requires the same docker-compose.yml pgvector setup as Stage 21/22
...
# ---------------------------------------------------------------------------
# Research Agent - web search specialist (copied from Stage 22, unchanged)
# ---------------------------------------------------------------------------
...
# document_chunks table, via pgvector), now scoped to the requesting user's
# own documents only. Stage 22's tool searched every uploaded document
# regardless of who uploaded it; this stage's version adds an
# InjectedState-sourced user_id the LLM cannot see or control, and filters
# the underlying query by it. See
# .claude/spec/stage23_user_document_isolation_spec.md §6-7 for the full
# reasoning.
# ---------------------------------------------------------------------------
...
class KnowledgeState(MessagesState):
    ...
    documents, so their subgraphs stay plain MessagesState (Stage 22's
    "changing what's INSIDE one specialist needs no changes elsewhere"
    lesson, applied here to a state schema instead of a tool binding).
    """
...
def search_uploaded_documents(
    ...
    # returns the closest k chunks whenever ANY embedded chunk owned by
    # this user_id exists, so the only reachable empty case is "this user
    # has no uploaded chunks with an embedding at all" - not "documents
    # exist but none are relevant". When documents exist but are a poor
    # topical match, the specialist LLM judges that itself from the
    # returned content, same as search_knowledge_base has always relied on
    # it to do.
    if not rows:
...
# ---------------------------------------------------------------------------
# Analysis Agent - safe-arithmetic specialist (copied from Stage 22,
# unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Supervisor + Critic - routing and review (copied from Stage 22, except
# CriticState now carries user_id so knowledge_node can hand it down into
# knowledge_graph.invoke(...) - see KnowledgeState above)
# ---------------------------------------------------------------------------
...
def knowledge_node(state: CriticState):
    ...
    so search_uploaded_documents's InjectedState argument can see it - this
    is the one line that changed here vs. Stage 22.
    """
    messages = state["messages"]
    if state.get("feedback"):
        messages = messages + [
...
# ---------------------------------------------------------------------------
# Planner + human approval (copied from Stage 22, except PlannerState now
# carries user_id - set once per /chat call and threaded down into
# research_subtask's supervisor_critic_graph.invoke(...) call; it survives
# the human_approval interrupt/resume the same way `question` already does,
# since PostgresSaver checkpoints the whole PlannerState dict)
# ---------------------------------------------------------------------------
...
def plan(state: PlannerState):
    ...
    # thread. user_id isn't reset here, same treatment as `question` -
    # both arrive fresh as graph.invoke() input on every /chat call.
    return {
        "subtasks": subtasks,
        "current_index": 0,
        "results": [],
...
def research_subtask(state: PlannerState):
    ...
    user_id is threaded into the inner graph's own state here (the one line
    that changed vs. Stage 22) so it can reach knowledge_node -> knowledge_
    graph -> search_uploaded_documents's InjectedState argument.
    """
...
# Same checkpointer setup as Stage 22: a PostgresSaver backed by a real
...
# ---------------------------------------------------------------------------
# Document upload tables. Created idempotently at module load, the same
# "safe to call every process start" convention as checkpointer.setup()
# just above. documents.user_id is new in this stage - see §5 of the spec
# for why it's declared inline here (for a genuinely fresh database) AND
# migrated separately below via ALTER TABLE (for this project's existing
# shared database, which already has documents rows from Stage 20-22
...
# Schema evolution for this project's existing shared database (Stage 21's
# own precedent: ALTER TABLE ... ADD COLUMN IF NOT EXISTS, not a migrations
# framework). NOT NULL DEFAULT 'default-user' backfills every pre-Stage-23
# documents row with that sentinel owner in the same statement that adds
# the column - there is never a row with a NULL/unowned user_id, so no
# filtering query anywhere needs to special-case one.
pg_conn.execute(
    "ALTER TABLE documents ADD COLUMN IF NOT EXISTS user_id TEXT NOT NULL DEFAULT 'default-user'"
)
# user_id becomes an equality filter on every retrieval query from this
# stage forward (search_uploaded_documents, /documents/search) - a plain
# B-tree index on it is standard practice, unlike the vector index Stage
# 21 §13 deliberately deferred (an ivfflat/hnsw index tunes an approximate
# structure at real cost; this is a cheap, ordinary index).
pg_conn.execute("CREATE INDEX IF NOT EXISTS idx_documents_user_id ON documents (user_id)")


# ---------------------------------------------------------------------------
# pgvector setup (Stage 21/22, unchanged). Idempotent, same "safe to run
# every process start" convention as checkpointer.setup() and the DDL
# above. The extension must exist before `vector` is a usable column type
# and before register_vector() can look up its type OID.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Document upload pipeline (Stage 20/21/22, unchanged except user_id is now
# a required part of every document): validate -> extract -> chunk -> embed
# -> store.
# ---------------------------------------------------------------------------
...
def _require_user_id(user_id: str) -> str:
    """Shared validation for /documents/upload, /documents/search, and
    /chat: user_id is required and must not be empty/whitespace-only.
    Mirrors this project's existing "present but empty" -> 400 pattern
    (Stage 21's `query` validation in /documents/search) - missing the
    field entirely is a different, automatic 422 from FastAPI/Pydantic,
...
# Copied from Stage 22, unchanged - the document routes below don't use it,
...
# embeddings are copied verbatim from Stage 22 - this stage only changes
# /chat, /documents/upload, and /documents/search (each gains a required
# user_id, per §3 of the spec).
# ---------------------------------------------------------------------------
...
app = FastAPI(
    title="Stage 23: Multi-Agent Research Assistant API with Per-User Document Isolation",
    description=(
        "FastAPI wrapper around Stage 22's planner + human-approval + "
        "supervisor/critic/specialist research graph plus document upload "
        "and semantic search, checkpointed to PostgreSQL. Every document "
        "is now owned by a caller-supplied user_id, and both retrieval "
        "paths (POST /documents/search and the Knowledge Agent's own "
        "tool) filter by it, so one user's uploads can never be returned "
        "to another user."
    ),
)
...
def chat(request: ChatRequest):
    ...
    user_id is new in this stage: it's validated here and passed into
    graph.invoke(...)'s initial state, where it's checkpointed as part of
    PlannerState and later threaded down into research_subtask ->
    supervisor_critic_graph -> knowledge_graph -> search_uploaded_documents
    (§7 of the spec). It is NOT resupplied on /approve or /reject - the
    value set here survives the human_approval interrupt/resume because
    it's part of the checkpointed state, the same way `question` already
    does.
    ...
    with _thread_lock(request.thread_id):
        ...
        try:
            result = graph.invoke(
                {"question": request.question, "user_id": user_id}, config=config
            )
        except Exception as exc:
...
def upload_document(file: UploadFile = File(...), user_id: str = Form(...)):
    ...
    Validation order: user_id -> extension -> read bytes -> empty -> size
    limit -> extract (content-based check #2) -> empty-extracted-text ->
    chunk -> embed -> store. Error-handling style matches every other
    route: a short, hand-written detail string on every HTTPException, the
    real exception printed server-side, never echoed to the client.
    """
...
def backfill_embeddings():
    ...
    backfill run. Deliberately NOT user-scoped (unchanged from Stage 21/22):
    it's a maintenance operation, and its response (BackfillResponse)
    contains only counts, never chunk content or filenames - there is
    nothing here for one user_id to leak to another. See §8 of the Stage 23
    spec.
...
def search_documents(request: SearchRequest):
    ...
    given document_id belongs to someone else (§6/§8 of the spec).
    """
    user_id = _require_user_id(request.user_id)
