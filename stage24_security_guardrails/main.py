"""
Stage 24: security and production guardrails around the existing document
upload, RAG search, FastAPI, and LangGraph flow - Stage 20-23 built that
pipeline assuming reasonably well-behaved input (valid PDFs, honest file
sizes, plain informational document text, a consistent user_id). Stage 24
doesn't add a new capability; it narrows what a malicious or malformed
input can make the existing pipeline do. See
.claude/spec/stage24_security_guardrails_spec.md for the full spec this
implements.
