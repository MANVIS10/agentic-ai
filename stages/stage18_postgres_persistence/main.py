"""
Stage 18: swap Stage 17's in-memory checkpointer for a PostgreSQL-backed
one, so a paused (or completed) conversation survives a program restart -
not just a `thread_id` switch within the same process, the way Stage 9's
JSON-file memory does for a single remembered fact.

New concept vs. Stage 1-17:
- Every earlier stage that needed a checkpointer used `MemorySaver`, which
  keeps all checkpointed state in the Python process's own memory - it's
  gone the instant the process exits. This stage replaces it with
  `PostgresSaver`, which writes each checkpoint to a real Postgres
  database, so the *outer planner graph's* state (the plan, the subtasks,
  results collected so far, and - most importantly - a paused-at-
  `human_approval` interrupt) can be resumed even after the Python process
  that created it has been killed and restarted, as long as the same
  `thread_id` and the same database are used again.
- Nothing else changes. This is Stage 17's exact graph, copied verbatim,
  with only the checkpointer construction swapped out - proving (the same
  way Stage 8 proved a compiled graph is "just a function call") that a
  checkpointer is a pluggable backend behind one interface
  (`checkpointer=...` at `.compile()` time), not something the rest of the
  graph needs to know or care about.

See this folder's README.md for how to start Postgres (`docker compose up
-d` from the repo root) and how to verify persistence across a restart.
"""
...
# ---------------------------------------------------------------------------
# Research Agent - web search specialist (copied from Stage 17, unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Knowledge Agent - local knowledge-base specialist (copied from Stage 17,
...
# ---------------------------------------------------------------------------
# Analysis Agent - safe-arithmetic specialist (copied from Stage 17,
# unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Supervisor + Critic - routing and review (copied from Stage 17, unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Planner + human approval (copied from Stage 17, unchanged)
# ---------------------------------------------------------------------------
...
# The only change from Stage 17: a PostgresSaver backed by a real database
# instead of MemorySaver's in-process dict, so this graph's checkpoints
# (the plan, subtasks, results-so-far, and any paused-at-human_approval
# interrupt) outlive the Python process. The connection is opened once at
# module scope, matching how MemorySaver() was created once at module scope
# - the checkpointer needs to already exist wherever `graph` is imported
# from (e.g. this stage's test file does `from main import graph`).
# autocommit=True: PostgresSaver issues its own transactions per checkpoint
# write: it doesn't want the driver batching writes into a longer-lived
# transaction. prepare_threshold=0 disables psycopg's statement-caching
# prepare step, which the LangGraph docs recommend against many Postgres
# setups (e.g. connection poolers) that don't support prepared statements.
...
def main():
    print("Stage 18: multi-agent research assistant with Postgres persistence.")
