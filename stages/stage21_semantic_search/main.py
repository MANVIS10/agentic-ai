"""
Stage 21: add embeddings and semantic vector search on top of Stage 20's
document upload pipeline, so an uploaded document's chunks become
searchable by meaning, not just stored as inert text rows.

New concept vs. Stage 1-20:
- Every prior embedding/search capability in this project
  ...
  stage stores embeddings durably in PostgreSQL instead, via the pgvector
  extension - the vectors themselves survive a process restart, the same
  way Stage 18 made checkpoints and Stage 20 made document_chunks durable.
- document_chunks (Stage 20's table) gains one new column, embedding
  vector(1536), added with ALTER TABLE ... ADD COLUMN IF NOT EXISTS - the
  first schema *evolution* in this repo, as opposed to first-time table
  creation (every earlier DDL in this project, including Stage 20's own,
  only ever used CREATE TABLE IF NOT EXISTS).
- Because Stage 20 and Stage 21 both write to the SAME Postgres database
  (this project runs one shared instance, not per-stage schemas - see
  docker-compose.yml), any document_chunks rows written before this stage
  existed have embedding = NULL. POST /documents/backfill-embeddings
  exists specifically to embed those rows after the fact - new uploads
  through this stage's own /documents/upload are embedded immediately and
  never need backfilling.
- This is deliberately search only - nothing here feeds search results
  into an LLM or wires a new tool onto the Knowledge Agent. See
  .claude/spec/stage21_semantic_search_spec.md for the full spec (Stage 22
  is expected to wrap POST /documents/search in a @tool the Knowledge
  Agent - or a new specialist - can call).
- Everything else - every specialist subgraph, the supervisor+critic
  ...
  /approve /reject /documents/upload routes - is copied verbatim from
  Stage 20. Nothing about the graph itself changed.

This stage also requires docker-compose.yml's Postgres image to support
pgvector (image: pgvector/pgvector:pg16, swapped from plain postgres:16) -
see this folder's README.md for the full setup/run instructions.
"""
...
# ---------------------------------------------------------------------------
# Research Agent - web search specialist (copied from Stage 20, unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Knowledge Agent - local knowledge-base specialist (copied from Stage 20,
...
# ---------------------------------------------------------------------------
# Analysis Agent - safe-arithmetic specialist (copied from Stage 20,
# unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Supervisor + Critic - routing and review (copied from Stage 20, unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Planner + human approval (copied from Stage 20, unchanged)
# ---------------------------------------------------------------------------
...
# Same checkpointer setup as Stage 20: a PostgresSaver backed by a real
...
# ---------------------------------------------------------------------------
# pgvector setup (Stage 21 - new). Idempotent, same "safe to run every
...
# register_vector() can look up its type OID - see the module docstring.
#
# ALTER TABLE ... ADD COLUMN IF NOT EXISTS (not CREATE TABLE IF NOT EXISTS)
# because document_chunks already exists - this is the first schema
# EVOLUTION in this repo, not first-time table creation. It also means a
# process running Stage 20's own (unmodified) main.py against this same
# database keeps working unchanged: its INSERT never mentions `embedding`,
# so new Stage 20 uploads simply leave that column NULL.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Document upload pipeline (Stage 20, unchanged): validate -> extract ->
# chunk. Storage happens inline in the upload_document() route below,
# since it needs the document_id generated there. Stage 21 adds embedding
# generation into that same route - see upload_document() itself.
# ---------------------------------------------------------------------------
...
# change for a multi-worker deployment). Copied from Stage 20, unchanged -
# the document routes below don't use it, since documents aren't scoped to
# a thread_id.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FastAPI layer. /health, /chat, /approve, /reject, /documents/upload are
# copied from Stage 20 (upload gains embedding generation - see below).
# POST /documents/backfill-embeddings and POST /documents/search
# (Stage 21 - new) are appended after them.
# ---------------------------------------------------------------------------
...
app = FastAPI(
    title="Stage 21: Multi-Agent Research Assistant API with Semantic Document Search",
    description=(
        "FastAPI wrapper around Stage 20's planner + human-approval + "
        "supervisor/critic/specialist research graph plus document upload, "
        "checkpointed to PostgreSQL, now with pgvector-backed embeddings "
        "and a POST /documents/search endpoint for semantic similarity "
        "search over uploaded document chunks."
    ),
)
...
def upload_document(file: UploadFile = File(...)):
    ...
    PDF/TXT/DOCX file. Stage 20 stopped at chunk+store; Stage 21 adds
    embedding generation right before storage, so every chunk this route
    writes already has its embedding attached - no backfill ever needed
    for uploads that go through this route.

    Validation order matches Stage 20 exactly - cheapest/no-I/O checks
    first: extension -> read bytes -> empty -> size limit -> extract
    (content-based check #2) -> empty-extracted-text -> chunk -> embed ->
    store. Error-handling style matches every other route: a short,
...
def backfill_embeddings():
    ...
    written by Stage 20 before this stage existed, or leftovers from a
    previous partial backfill run (both stages share one Postgres
    database, so Stage 20 rows are visible here too).
...
def search_documents(request: SearchRequest):
    """Semantic similarity search over embedded document_chunks. Search
    only - nothing here feeds results into an LLM or answers a question
    (see the module docstring). An empty results list is a valid 200, not
    an error, whenever nothing clears similarity_threshold or no chunk in
    scope has an embedding yet.
    """
